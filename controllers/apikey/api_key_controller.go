...
// +kubebuilder:rbac:groups=apikey.datadog.kubeform.com,resources=apikeys,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apikey.datadog.kubeform.com,resources=apikeys/status,verbs=get;update;patch

func (r *ApiKeyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("apikey", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ApiKey")
...
func (r *ApiKeyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &apikeyv1alpha1.ApiKey{}); err != nil {
			klog.Error(err, "unable to set up auditor", apikeyv1alpha1.ApiKey{}.APIVersion, apikeyv1alpha1.ApiKey{}.Kind)
			return err
		}
	}
...
func (r *ApiKeyReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for ApiKey")
			return nil
		}
		...
		resourceList := &apikeyv1alpha1.ApiKeyList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list ApiKey")
			return nil
		}
