...
// DowntimeReconciler reconciles a Downtime object
type DowntimeReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=downtime.datadog.kubeform.com,resources=downtimes,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=downtime.datadog.kubeform.com,resources=downtimes/status,verbs=get;update;patch

func (r *DowntimeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("downtime", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Downtime")
...
func (r *DowntimeReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &downtimev1alpha1.Downtime{}); err != nil {
			klog.Error(err, "unable to set up auditor", downtimev1alpha1.Downtime{}.APIVersion, downtimev1alpha1.Downtime{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&downtimev1alpha1.Downtime{}).
