...
// CorrectionReconciler reconciles a Correction object
...
// +kubebuilder:rbac:groups=slo.datadog.kubeform.com,resources=corrections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=slo.datadog.kubeform.com,resources=corrections/status,verbs=get;update;patch

func (r *CorrectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("correction", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Correction")
...
func (r *CorrectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &slov1alpha1.Correction{}); err != nil {
			klog.Error(err, "unable to set up auditor", slov1alpha1.Correction{}.APIVersion, slov1alpha1.Correction{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&slov1alpha1.Correction{}).
