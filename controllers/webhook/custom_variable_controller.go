...
// CustomVariableReconciler reconciles a CustomVariable object
type CustomVariableReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=webhook.datadog.kubeform.com,resources=customvariables,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=webhook.datadog.kubeform.com,resources=customvariables/status,verbs=get;update;patch

func (r *CustomVariableReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("customvariable", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch CustomVariable")
...
func (r *CustomVariableReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &webhookv1alpha1.CustomVariable{}); err != nil {
			klog.Error(err, "unable to set up auditor", webhookv1alpha1.CustomVariable{}.APIVersion, webhookv1alpha1.CustomVariable{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&webhookv1alpha1.CustomVariable{}).
...
func (r *CustomVariableReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(fmt.Errorf("expected a Secret but go a %T", o), "failed to get secret for CustomVariable")
			return nil
		}
		...
		resourceList := &webhookv1alpha1.CustomVariableList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list CustomVariable")
			return nil
		}
