...
// LevelObjectiveReconciler reconciles a LevelObjective object
type LevelObjectiveReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=service.datadog.kubeform.com,resources=levelobjectives,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=service.datadog.kubeform.com,resources=levelobjectives/status,verbs=get;update;patch

func (r *LevelObjectiveReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("levelobjective", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch LevelObjective")
...
func (r *LevelObjectiveReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &servicev1alpha1.LevelObjective{}); err != nil {
			klog.Error(err, "unable to set up auditor", servicev1alpha1.LevelObjective{}.APIVersion, servicev1alpha1.LevelObjective{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&servicev1alpha1.LevelObjective{}).
