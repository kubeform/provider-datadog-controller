...
// +kubebuilder:rbac:groups=metric.datadog.kubeform.com,resources=metadata,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=metric.datadog.kubeform.com,resources=metadata/status,verbs=get;update;patch

func (r *MetadataReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("metadata", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Metadata")
...
func (r *MetadataReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &metricv1alpha1.Metadata{}); err != nil {
			klog.Error(err, "unable to set up auditor", metricv1alpha1.Metadata{}.APIVersion, metricv1alpha1.Metadata{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&metricv1alpha1.Metadata{}).
