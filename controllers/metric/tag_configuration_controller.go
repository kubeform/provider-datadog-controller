...
// TagConfigurationReconciler reconciles a TagConfiguration object
type TagConfigurationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=metric.datadog.kubeform.com,resources=tagconfigurations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=metric.datadog.kubeform.com,resources=tagconfigurations/status,verbs=get;update;patch

func (r *TagConfigurationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("tagconfiguration", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch TagConfiguration")
...
func (r *TagConfigurationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &metricv1alpha1.TagConfiguration{}); err != nil {
			klog.Error(err, "unable to set up auditor", metricv1alpha1.TagConfiguration{}.APIVersion, metricv1alpha1.TagConfiguration{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&metricv1alpha1.TagConfiguration{}).
