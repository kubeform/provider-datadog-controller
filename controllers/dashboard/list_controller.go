...
// ListReconciler reconciles a List object
...
// +kubebuilder:rbac:groups=dashboard.datadog.kubeform.com,resources=lists,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=dashboard.datadog.kubeform.com,resources=lists/status,verbs=get;update;patch

func (r *ListReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("list", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch List")
...
func (r *ListReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &dashboardv1alpha1.List{}); err != nil {
			klog.Error(err, "unable to set up auditor", dashboardv1alpha1.List{}.APIVersion, dashboardv1alpha1.List{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&dashboardv1alpha1.List{}).
