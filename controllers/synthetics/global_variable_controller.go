...
// +kubebuilder:rbac:groups=synthetics.datadog.kubeform.com,resources=globalvariables,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synthetics.datadog.kubeform.com,resources=globalvariables/status,verbs=get;update;patch

func (r *GlobalVariableReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("globalvariable", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch GlobalVariable")
...
func (r *GlobalVariableReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &syntheticsv1alpha1.GlobalVariable{}); err != nil {
			klog.Error(err, "unable to set up auditor", syntheticsv1alpha1.GlobalVariable{}.APIVersion, syntheticsv1alpha1.GlobalVariable{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&syntheticsv1alpha1.GlobalVariable{}).
...
func (r *GlobalVariableReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for GlobalVariable")
			return nil
		}
		...
		resourceList := &syntheticsv1alpha1.GlobalVariableList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list GlobalVariable")
			return nil
		}
