...
// SyntheticstestReconciler reconciles a Syntheticstest object
type SyntheticstestReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=synthetics.datadog.kubeform.com,resources=syntheticstests,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synthetics.datadog.kubeform.com,resources=syntheticstests/status,verbs=get;update;patch

func (r *SyntheticstestReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("syntheticstest", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Syntheticstest")
...
func (r *SyntheticstestReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &syntheticsv1alpha1.Syntheticstest{}); err != nil {
			klog.Error(err, "unable to set up auditor", syntheticsv1alpha1.Syntheticstest{}.APIVersion, syntheticsv1alpha1.Syntheticstest{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&syntheticsv1alpha1.Syntheticstest{}).
...
func (r *SyntheticstestReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Syntheticstest")
			return nil
		}
		...
		resourceList := &syntheticsv1alpha1.SyntheticstestList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Syntheticstest")
			return nil
		}
