...
// PrivateLocationReconciler reconciles a PrivateLocation object
type PrivateLocationReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=synthetics.datadog.kubeform.com,resources=privatelocations,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=synthetics.datadog.kubeform.com,resources=privatelocations/status,verbs=get;update;patch

func (r *PrivateLocationReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("privatelocation", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PrivateLocation")
...
func (r *PrivateLocationReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &syntheticsv1alpha1.PrivateLocation{}); err != nil {
			klog.Error(err, "unable to set up auditor", syntheticsv1alpha1.PrivateLocation{}.APIVersion, syntheticsv1alpha1.PrivateLocation{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&syntheticsv1alpha1.PrivateLocation{}).
...
func (r *PrivateLocationReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for PrivateLocation")
			return nil
		}
		...
		resourceList := &syntheticsv1alpha1.PrivateLocationList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list PrivateLocation")
			return nil
		}
