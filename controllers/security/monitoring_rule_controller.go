...
// MonitoringRuleReconciler reconciles a MonitoringRule object
type MonitoringRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=security.datadog.kubeform.com,resources=monitoringrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=security.datadog.kubeform.com,resources=monitoringrules/status,verbs=get;update;patch

func (r *MonitoringRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("monitoringrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MonitoringRule")
...
func (r *MonitoringRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &securityv1alpha1.MonitoringRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", securityv1alpha1.MonitoringRule{}.APIVersion, securityv1alpha1.MonitoringRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&securityv1alpha1.MonitoringRule{}).
