...
// MonitoringDefaultRuleReconciler reconciles a MonitoringDefaultRule object
type MonitoringDefaultRuleReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=security.datadog.kubeform.com,resources=monitoringdefaultrules,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=security.datadog.kubeform.com,resources=monitoringdefaultrules/status,verbs=get;update;patch

func (r *MonitoringDefaultRuleReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("monitoringdefaultrule", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch MonitoringDefaultRule")
...
func (r *MonitoringDefaultRuleReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &securityv1alpha1.MonitoringDefaultRule{}); err != nil {
			klog.Error(err, "unable to set up auditor", securityv1alpha1.MonitoringDefaultRule{}.APIVersion, securityv1alpha1.MonitoringDefaultRule{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&securityv1alpha1.MonitoringDefaultRule{}).
