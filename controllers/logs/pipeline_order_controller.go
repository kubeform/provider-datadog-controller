...
// PipelineOrderReconciler reconciles a PipelineOrder object
type PipelineOrderReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logs.datadog.kubeform.com,resources=pipelineorders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logs.datadog.kubeform.com,resources=pipelineorders/status,verbs=get;update;patch

func (r *PipelineOrderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("pipelineorder", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PipelineOrder")
...
func (r *PipelineOrderReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logsv1alpha1.PipelineOrder{}); err != nil {
			klog.Error(err, "unable to set up auditor", logsv1alpha1.PipelineOrder{}.APIVersion, logsv1alpha1.PipelineOrder{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logsv1alpha1.PipelineOrder{}).
