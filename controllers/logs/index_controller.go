...
// +kubebuilder:rbac:groups=logs.datadog.kubeform.com,resources=indices,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logs.datadog.kubeform.com,resources=indices/status,verbs=get;update;patch

func (r *IndexReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("index", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Index")
...
func (r *IndexReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logsv1alpha1.Index{}); err != nil {
			klog.Error(err, "unable to set up auditor", logsv1alpha1.Index{}.APIVersion, logsv1alpha1.Index{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logsv1alpha1.Index{}).
