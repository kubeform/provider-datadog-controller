...
// ArchiveOrderReconciler reconciles a ArchiveOrder object
type ArchiveOrderReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logs.datadog.kubeform.com,resources=archiveorders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logs.datadog.kubeform.com,resources=archiveorders/status,verbs=get;update;patch

func (r *ArchiveOrderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("archiveorder", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch ArchiveOrder")
...
func (r *ArchiveOrderReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logsv1alpha1.ArchiveOrder{}); err != nil {
			klog.Error(err, "unable to set up auditor", logsv1alpha1.ArchiveOrder{}.APIVersion, logsv1alpha1.ArchiveOrder{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logsv1alpha1.ArchiveOrder{}).
