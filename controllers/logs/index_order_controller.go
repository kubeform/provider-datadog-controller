...
// IndexOrderReconciler reconciles a IndexOrder object
type IndexOrderReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logs.datadog.kubeform.com,resources=indexorders,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logs.datadog.kubeform.com,resources=indexorders/status,verbs=get;update;patch

func (r *IndexOrderReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("indexorder", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IndexOrder")
...
func (r *IndexOrderReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logsv1alpha1.IndexOrder{}); err != nil {
			klog.Error(err, "unable to set up auditor", logsv1alpha1.IndexOrder{}.APIVersion, logsv1alpha1.IndexOrder{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logsv1alpha1.IndexOrder{}).
