...
// IntegrationPipelineReconciler reconciles a IntegrationPipeline object
type IntegrationPipelineReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=logs.datadog.kubeform.com,resources=integrationpipelines,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logs.datadog.kubeform.com,resources=integrationpipelines/status,verbs=get;update;patch

func (r *IntegrationPipelineReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("integrationpipeline", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch IntegrationPipeline")
...
func (r *IntegrationPipelineReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logsv1alpha1.IntegrationPipeline{}); err != nil {
			klog.Error(err, "unable to set up auditor", logsv1alpha1.IntegrationPipeline{}.APIVersion, logsv1alpha1.IntegrationPipeline{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logsv1alpha1.IntegrationPipeline{}).
