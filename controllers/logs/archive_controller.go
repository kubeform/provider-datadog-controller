...
// +kubebuilder:rbac:groups=logs.datadog.kubeform.com,resources=archives,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=logs.datadog.kubeform.com,resources=archives/status,verbs=get;update;patch
...
func (r *ArchiveReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Archive")
...
func (r *ArchiveReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &logsv1alpha1.Archive{}); err != nil {
			klog.Error(err, "unable to set up auditor", logsv1alpha1.Archive{}.APIVersion, logsv1alpha1.Archive{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&logsv1alpha1.Archive{}).
