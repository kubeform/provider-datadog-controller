...
// +kubebuilder:rbac:groups=integration.datadog.kubeform.com,resources=pagerduties,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=integration.datadog.kubeform.com,resources=pagerduties/status,verbs=get;update;patch

func (r *PagerdutyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("pagerduty", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch Pagerduty")
...
func (r *PagerdutyReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &integrationv1alpha1.Pagerduty{}); err != nil {
			klog.Error(err, "unable to set up auditor", integrationv1alpha1.Pagerduty{}.APIVersion, integrationv1alpha1.Pagerduty{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&integrationv1alpha1.Pagerduty{}).
...
func (r *PagerdutyReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for Pagerduty")
			return nil
		}
		...
		resourceList := &integrationv1alpha1.PagerdutyList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list Pagerduty")
			return nil
		}
