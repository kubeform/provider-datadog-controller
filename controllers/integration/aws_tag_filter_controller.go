...
// AwsTagFilterReconciler reconciles a AwsTagFilter object
type AwsTagFilterReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=integration.datadog.kubeform.com,resources=awstagfilters,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=integration.datadog.kubeform.com,resources=awstagfilters/status,verbs=get;update;patch

func (r *AwsTagFilterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("awstagfilter", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AwsTagFilter")
...
func (r *AwsTagFilterReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &integrationv1alpha1.AwsTagFilter{}); err != nil {
			klog.Error(err, "unable to set up auditor", integrationv1alpha1.AwsTagFilter{}.APIVersion, integrationv1alpha1.AwsTagFilter{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&integrationv1alpha1.AwsTagFilter{}).
