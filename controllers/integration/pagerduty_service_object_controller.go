...
// PagerdutyServiceObjectReconciler reconciles a PagerdutyServiceObject object
type PagerdutyServiceObjectReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=integration.datadog.kubeform.com,resources=pagerdutyserviceobjects,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=integration.datadog.kubeform.com,resources=pagerdutyserviceobjects/status,verbs=get;update;patch

func (r *PagerdutyServiceObjectReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("pagerdutyserviceobject", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch PagerdutyServiceObject")
...
func (r *PagerdutyServiceObjectReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &integrationv1alpha1.PagerdutyServiceObject{}); err != nil {
			klog.Error(err, "unable to set up auditor", integrationv1alpha1.PagerdutyServiceObject{}.APIVersion, integrationv1alpha1.PagerdutyServiceObject{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&integrationv1alpha1.PagerdutyServiceObject{}).
...
func (r *PagerdutyServiceObjectReconciler) SensitiveSecretWatch(ctx context.Context) handler.MapFunc {
	...
	return func(o client.Object) []ctrl.Request {
		...
		if !ok {
			log.Error(errors.Errorf("expected a Secret but go a %T", o), "failed to get secret for PagerdutyServiceObject")
			return nil
		}
		...
		resourceList := &integrationv1alpha1.PagerdutyServiceObjectList{}
		if err := r.List(ctx, resourceList, client.InNamespace(secNamespace)); err != nil {
			log.Error(err, "failed to list PagerdutyServiceObject")
			return nil
		}
