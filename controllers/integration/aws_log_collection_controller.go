...
// AwsLogCollectionReconciler reconciles a AwsLogCollection object
type AwsLogCollectionReconciler struct {
	client.Client
	Log    logr.Logger
...
// +kubebuilder:rbac:groups=integration.datadog.kubeform.com,resources=awslogcollections,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=integration.datadog.kubeform.com,resources=awslogcollections/status,verbs=get;update;patch

func (r *AwsLogCollectionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("awslogcollection", req.NamespacedName)

	var unstructuredObj unstructured.Unstructured
	unstructuredObj.SetGroupVersionKind(r.Gvk)

	if err := r.Get(ctx, req.NamespacedName, &unstructuredObj); err != nil {
		log.Error(err, "unable to fetch AwsLogCollection")
...
func (r *AwsLogCollectionReconciler) SetupWithManager(ctx context.Context, mgr ctrl.Manager, auditor *auditlib.EventPublisher, restrictToNamespace string) error {
	if auditor != nil {
		if err := auditor.SetupWithManager(ctx, mgr, &integrationv1alpha1.AwsLogCollection{}); err != nil {
			klog.Error(err, "unable to set up auditor", integrationv1alpha1.AwsLogCollection{}.APIVersion, integrationv1alpha1.AwsLogCollection{}.Kind)
			return err
		}
	}

	return ctrl.NewControllerManagedBy(mgr).
		For(&integrationv1alpha1.AwsLogCollection{}).
